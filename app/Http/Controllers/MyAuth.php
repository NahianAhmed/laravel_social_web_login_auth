...
public function VerifyLogin(Request $request){
  ...
  /**
    ...
    if($data = UserTable::where('password',$user->id)->first())
    {
        if($data->email == $user->email && $data->password==$user->id){
            $user_type=$data->user_type;
            Session::put('UserSession',$data->name);
            Session::put('User_email',$data->email);
            Session::put('UserType',$user_type);
            ...
            return redirect('/login')->with('msg','Problem in your social account');
        }
